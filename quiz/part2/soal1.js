...
//     }
//   }
//   faktorCount = Math.ceil(faktorCount / 2);
...
//       if (i * j == angka) {
//         if (faktor[faktor.length - 1] == Number(`${j}${i}`)) {
//           stop = true;
//         } else {
//           faktor.push(Number(String(i) + String(j)));
//         }
//       }
//     }
//     if (stop) break;
...
function digitPerkalianMinimum(angka) {
  ...
  for (let i = 1; i <= Math.sqrt(angka); i++) {
    if (angka % i === 0) {
      ...
      let gabunganDigit = (String(i) + String(pasangan)).length;

      minDigit = gabunganDigit;
    }
  }

...
objek-objek barang dari toko X tersebut yang berisikan info nama barang, siapa saja yang membeli, 
...
function countProfit(shoppers) {
  ...
  // Inisialisasi hasil penjualan per produk
  ...
  // Proses transaksi dari shoppers
  for (let outer = 0; outer < shoppers.length; outer++) {
    let costumerName = shoppers[outer].name;
    let targetProduct = shoppers[outer].product;
    let totalTarget = shoppers[outer].amount;

    for (let inner = 0; inner < listBarang.length; inner++) {
      let productName = listBarang[inner][0];
      let productPrice = listBarang[inner][1];

      if (productName == targetProduct) {
        if (profit[inner].leftOver >= totalTarget) {
          profit[inner].shoppers.push(costumerName);
          profit[inner].leftOver -= totalTarget;
          profit[inner].totalProfit += totalTarget * productPrice;
        }
      }
    }
  }
